<?php
function filia_form_system_theme_settings_alter(&$form, &$form_state) {
 ...
 $form['background_container']['body_bg_classes'] = array(
  ...
  $form['layout_container'] = array(
    '#type' => 'fieldset',
    '#title' => t('Layout'),
    '#description' => t('Use these settings to change the layout.'),
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  );
  ...
  $form['layout_container']['layout_classes'] = array(
    '#type'          => 'radios',
    '#title'         => t('Choose the layout for the site'),
    '#default_value' => theme_get_setting('layout_classes'),
    '#options'       => array(
	  'wide' => t('Wide - <strong><em>Default</em></strong>'),
      'boxed' => t('Boxed'),
    ),
  );
  
  // Attach custom submit handler to the form
  $form['#submit'][] = 'filia_settings_submit';
  $form['#validate'][] = 'filia_settings_validate';
}

function filia_settings_submit($form, &$form_state) {
  $settings = array();
...
function filia_settings_validate($form, &$form_state) {
