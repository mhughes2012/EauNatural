...
function filia_js_alter(&$javascript) {
  // Update jquery version for non-administration pages

}

function filia_preprocess_html(&$vars) {
  ...
  $vars['layout_classes'] = theme_get_setting('layout_classes'); 
...
function filia_preprocess_page(&$vars) { 
...
function filia_preprocess_block(&$vars) {
...
function filia_region_has_block($region) {
...
function filia_get_span($block_count, $block_id, $count_sidebars) {
  ...
  if ($count_sidebars) {
    ...
    if (filia_region_has_block('sidebar_first')) {
      $available_width = $available_width - 0;
    }

    if (filia_region_has_block('sidebar_second')) {
      $available_width = $available_width - 0;
    }
  }
 ...
 */
function filia_status_messages($variables) {
...
function filia_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'search_block_form') {
    ...
    $form['actions']['submit']['#src'] = drupal_get_path('theme', 'filia') . '/img/searchbutton.png';
  }
}

/* Returns HTML for primary and secondary local tasks. */
function filia_menu_local_tasks(&$vars) {
  $output = '';
...
function filia_menu_local_task($vars) {
...
function filia_menu_link(array $vars) {

  $element = $vars['element'];

  if (filia_is_in_nav_menu($element)) {
    $sub_menu = '';
...
function filia_item_list($variables) {
...
function filia_is_in_nav_menu($element) {
  ...
  if (!isset($nav_theming_functions)) {
    ...
    $nav_theming_functions = array_map('filia_block_id_to_function_name', $ids);
...
function filia_block_id_to_function_name ($id) {
